...
const PendingElement = ({ type, request }) => {
	...
	const approveRequest = (type, request) => {
		if (type === "donation") {
			...
			acceptApplication({ donationId: request?.donation?._id, applicationId: request?._id })
				.unwrap()
				.then((response) => {
			...
			<Grid container>
				<Grid item xs={isExtraSmall ? 12 : 5} sm={3} md={3} lg={2.5} xl={2} padding="8px">
					<img
						...
						loading="lazy"
						src={request.donationImage || request.donation?.donationImage}
						alt={request.donationName || request.donation?.donationName}
					/>
				</Grid>
					...
					>
						...
						>
							<Typography variant="body1" fontSize={14}>
								...
								{request.donationName || request.donation?.donationName}
							</Typography>
							<Typography variant="body1" fontSize={14}>
