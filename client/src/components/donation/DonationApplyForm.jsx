...
import DonationExcerpt from "./DonationExcerpt";
import { useDispatch } from "react-redux";
import { openCart } from "../../features/drawer/drawerSlice";
import { useApplyDonationMutation } from "../../features/donations/donationsSlice";

const DonationApplyForm = () => {
	const theme = useTheme();
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const { state } = useLocation();
	const [applyDonation, applyInfo] = useApplyDonationMutation();
	...
	const schema = yup
		.object({
			...
			reason: yup
				.string()
				.min(25, "Reason for this donation must be at least 25 characters")
				.max(500, "Reason for this donation must be at most 500 characters")
				.required("Reason for this donation is required"),
		})
		.required();
	...
	const onSubmit = (data) => {
		applyDonation({ donation: state._id, ...data, status: "pending" })
			.unwrap()
			.then((response) => {
				if (response.msg === "apply_successful") {
					toast.success("Successfully get the form data");
					dispatch(openCart());
					return navigate("/donations", { replace: true });
				}
	...
	return (
		<>
			<Box component="form" p="20px" onSubmit={handleSubmit(onSubmit)}>
					...
					<Grid item xs={12} sm={6}>
						...
						>
							<InputLabel htmlFor="reason">
								Explain us, Why you need this donation...
							</InputLabel>
				...
				<Button
					onClick={() => navigate("/donations", { replace: true })}
			...
			<Box sx={{ mt: 3 }}>
				...
				<Grid container>
					<Grid item xs={12} md={6} lg={4} sx={{ m: { xs: "14px", sm: "20px" } }}>
						<DonationExcerpt donation={state} />
					</Grid>
				</Grid>
			</Box>
		</>
...
export default DonationApplyForm;
