...
const DonationCartDetails = ({ donation }) => {
	...
	const handleCount = (type) => {
		if (type === "inc" && count < donation?.dosages) {
		...
		<>
			...
			<Typography sx={{ display: "flex", alignItems: "center", mt: 1 }} fontSize="14px">
				...
				<span style={{ color: "#727272" }}>
					{donationRatingInfo.data?.totalRating || 0} reviews
				</span>
			</Typography>
			<Stack direction="row" gap={1.5} marginY={2.5}>
				...
				<Button
					fullWidth
					onClick={() =>
						navigate(`/donations/apply/${donation.id}`, {
							state: { ...donation, count },
						})
					}
					sx={{
...
export default DonationCartDetails;
