...
const CartItem = ({ cartItem }) => {
	...
	const cancelApplication = () => {
		rejectApplication(cartItem?.donation._id)
			.unwrap()
			.then((response) => {
			...
			<Stack direction="row" sx={{ pt: 1.6, pb: 1.8 }} spacing={1.5}>
				<Box sx={{ height: "90px", width: "100px", position: "relative" }}>
					<img
						...
						loading="lazy"
						src={cartItem?.donation?.donationImage}
						alt={cartItem?.donation?.donationName}
					/>
				...
				<Box>
					<Typography mb={0.4}>{cartItem?.donation?.donationName}</Typography>
