...
exports.getDashboardChartDataController = asyncHandler(async (req, res) => {
	const [totalDonations, donatedDonations] = await Promise.all([
		Donation.aggregate([
	...
	res.status(200).json({
		total: totalDonations[0].total,
		donation: totalDonations[0].total - totalDonations[0].remaining, // get donated donation
		donationReceived: donatedDonations[0].received,
	});
});
